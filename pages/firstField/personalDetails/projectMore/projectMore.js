...
Page({
  ...
  sumApiNetProject: function() {
    ...
    api.fetchRequest(orgType + '/' + that.data.scholarId + '/projects', {
      page: page,
      num: num
    }).then(function(res) {
      ...
      if (code == 200) {
        if (res.data.data.content.length != 0) {
          for (let i = 0; i < res.data.data.content.length; i++) {
            let _projectMsg = that.data.projectMsg
            _projectMsg.push(res.data.data.content[i])
            that.setData({
              projectMsg: _projectMsg,
              isHideLoadMore: true,
              isLoadMore: true,
            })
          }
  ...
  sumApiNetProjectTab: function() {
    ...
    api.fetchRequest(orgType + '/projectFilterField?page=' + page + '&num=' + num, {
  ...
  loadMore: function(e) {
    ...
    if (tabtype == 0) {
      ...
      that.sumApiNetProjectTab()
    }
  },
  /**
   * 项目详情
   */
  projectBitp: function(e) {
    let that = this
    var _index = e.currentTarget.dataset.index;
    let _id = that.data.projectMsg[_index].id
    wx.navigateTo({
